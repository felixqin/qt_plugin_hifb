...
typedef struct
{
    unsigned int bAlphaEnable;   /**<  alpha enable flag */
    unsigned int bAlphaChannel;  /**<  alpha channel enable flag */
    unsigned char u8Alpha0;         /**<  alpha0 value, used in ARGB1555 */
    unsigned char u8Alpha1;         /**<  alpha1 value, used in ARGB1555 */
    unsigned char u8GlobalAlpha;    /**<  global alpha value */
...
bool QLinuxFbScreen::initialize()
{
    ...
    if (userGeometry.isValid()) {
        ...
        vinfo.xres = userGeometry.width();
        vinfo.yres = userGeometry.height();
        vinfo.xres_virtual = userGeometry.width();
        vinfo.yres_virtual = userGeometry.height();
        vinfo.transp = s_a16;
        vinfo.red = s_r16;
        vinfo.green = s_g16;
        vinfo.blue = s_b16;
        ...
        if (ioctl(mFbFd, FBIOPUT_VSCREENINFO, &vinfo)) {
            qErrnoWarning(errno, "Error writing variable information");
            return false;
        }
    }
    ...
    if (ioctl(mFbFd, FBIOGET_ALPHA_HIFB, &stAlpha) != 0) {
        qErrnoWarning(errno, "Error reading alpha information");
        return false;
    }

    stAlpha.u8Alpha0 = 0x80;
    if (ioctl(mFbFd, FBIOPUT_ALPHA_HIFB, &stAlpha) != 0) {
        qErrnoWarning(errno, "Error writing alpha information");
        return false;
    }
    ...
    printf("000 fb offset(%d) size(%dx%d) bpl(%d) fmt(%d) depth(%d)\n", mMmap.offset, geometry.width(), geometry.height(), mBytesPerLine, mFormat, mDepth);
